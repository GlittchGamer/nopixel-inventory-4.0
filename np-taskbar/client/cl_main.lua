...
function updateGui(sentLength,taskID,namesent)
    ...
    exports["rp-ui"]:sendAppEvent("taskbar", {
        update = true,
        value = sentLength,
    })
end
...
function closeGui()
    ...
    exports["qb-interface"]:sendAppEvent("taskbar", {
        display = false,
    })
end
...
AddEventHandler('coffee:drink', function()
    TriggerEvent('qb-hud:ChangeThirst', 10)
...
RegisterNetEvent('wine:drink')
AddEventHandler('wine:drink', function()
    TriggerEvent('qb-hud:ChangeThirst', 20)
    TriggerEvent("DoLongHudText", "I seem to be doing actions a little faster for a short period of time", 2)
    if coffeetimer > 0 then
        coffeetimer = 250000
        TriggerEvent("Evidence:StateSet",27,6000)
        return
...
RegisterNetEvent('RingBuff')
AddEventHandler('RingBuff', function(level)
...
function taskBar(length,name,runCheck,keepWeapon,vehicle,vehCheck,cb)
    ...
    if coffee > 0 then 
        length = math.ceil(length * coffee)
    elseif coffeetimer > 0 then
...
function handheld() 
  ...
  function generalInventory()
    if not insidePrompt and not inventoryDisabled and not forceInventoryDisabled and not exports['np-taskbar']:pInInv() then
  ...
  local function hasPhone()
    return exports["inventory"]:hasEnoughOfItem("mobilephone", 1, false, true) or
        exports["inventory"]:hasEnoughOfItem("stoleniphone", 1, false, true) or
        exports["inventory"]:hasEnoughOfItem("stolens8", 1, false, true) or
        exports["inventory"]:hasEnoughOfItem("stolennokia", 1, false, true) or
        exports["inventory"]:hasEnoughOfItem("stolenpixel3", 1, false, true) or
        exports["inventory"]:hasEnoughOfItem("boomerphone", 1, false, true)
  end
  ...
  function generalPhone()
    if not insidePrompt and hasPhone() and canUsePhone() then
      ...
      -- exports["rp-ui"]:openApplication("phone", {
...
Citizen.CreateThread(function()
    exports["np-keybinds"]:registerKeyMapping("", "Radio", "Open", "+handheld", "-handheld", "I")
