...
export const AccessoriesLeft = () => {
    ...
    const [draggingSlotId, setDraggingSlotId] = createSignal<string | null>(null);
    ...
    const handleMouseDown = (e: any, pSlot: any) => {
        if (e.button !== 0) return;
        if (!pSlot.item) return;

        if (PlayerState.settings.holdToDrag && pSlot.item && !movingSplitItem()) {
            setDraggingSlotId(pSlot.id);
            setDraggingItem({
                isDragging: true,
                Item: itemList[pSlot.item.itemId],
                quantity: !pSlot.item.amount ? 1 : pSlot.item.amount,
                fromSlot: pSlot.id,
                fromInventory: 'clothing'
            });
    ...
    const handleMouseUp = (e: any) => {
        if (draggingItem.isDragging && !movingSplitItem()) {
            ...
            nuiAction('itemDrag', {
                ...
                source: PlayerState.character.id
            });
    ...
    const handleClick = (pSlot: any) => {
        if (!PlayerState.settings.holdToDrag && pSlot.item) {
            if (draggingItem.isDragging && !movingSplitItem()) {
                setDraggingSlotId(null);
                setDraggingItem({
                    isDragging: false,
                ...
                >
                    {(pSlot: any) => (
                        ...
                        >
                            <div>
                                <div
                                    class={`box-c relative h-24 w-24 rounded ${draggingSlotId() === pSlot.id ? 'bg-black opacity-50' : ''} ${draggingSlotId() === pSlot.id ? '' : '[background:radial-gradient(83%_83%_at_50%_50%,_rgba(180,_180,_180,_0.15),_rgba(120,_120,_120,_0.05))]'} [border:1px_solid_transparent] hover:[${draggingItem.item !== null ? calculateBackgroundColor(pSlot.acceptedItems, draggingItem.Item) : 'background:radial-gradient(83%_83%_at_50%_50%,_rgba(0,_248,_185,_0.15),_rgba(0,_102,_76,_0.25))'}] hover:[${draggingItem.Item !== null ? calculateBorderColor(pSlot.acceptedItems, draggingItem.Item) : 'border:1px_solid_#00F8B9'}]`}
                                    onClick={() => handleClick(pSlot)}
                                    onMouseDown={(e) => handleMouseDown(e, pSlot)}
                                    onContextMenu={(e) => {
                                        e.preventDefault();
                                        if (pSlot.item) {
                                            setHoveringItem({
                                    ...
                                    onMouseEnter={() => {
                                        setActiveSlot(pSlot.id);
                                        if (!hoveringItem.frozenPosition && pSlot.item !== null) {
                                            setHoveringItem({
                                                show: true,
                                                Information: pSlot.item,
                                                isSideSlot: true
