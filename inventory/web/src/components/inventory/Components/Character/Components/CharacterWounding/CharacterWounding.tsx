...
import { fetchNui } from "../../../../../../lib/nui/functions/FetchNui";
...
const fetchWoundState = async () => {
    try {
        const response = await fetchNui('getWoundState', {});
        return response;
    } catch (error) {
        console.error('Error fetching wound state:', error);
        return {
            wounds: {
                head: {
                    damage: 0,
...
export const CharacterWounding = () => {
    ...
    const [woundState, { refetch }] = createResource(fetchWoundState);
    
    createEffect(() => {
        if (!woundState.loading) {
            // console.log('Wound state data loaded:', JSON.stringify(woundState()));
        }
    });

    const handleWoundHover = (bodyPart: any) => {
        try {
            setHoveringWound(true);
            setWoundPosition(bodyPart === 'leftArm' || bodyPart === 'leftLeg' ? 'left' : 'right');
            
            const state = woundState();
            
            if (state && state.wounds && state.wounds[bodyPart]) {
                try {
                    setActiveWoundData(state.wounds[bodyPart]);
                } catch (innerError) {
                    console.error("Error specifically in setActiveWoundData:", innerError);
                }
            } else {
                // console.error(`No wound data found for ${bodyPart}`);
                setActiveWoundData(null);
            }
        } catch (error) {
            console.error("Error in handleWoundHover:", error);
    ...
    return (
        <>
            ...
            <svg
                ...
                >
                    <path
                        ...
                        onMouseEnter={() => handleWoundHover('head')}
                    ...
                    <path
                        ...
                        style="fill: rgba(255, 103, 103, 0);"
                        onMouseEnter={() => handleWoundHover('body')}
                        onMouseLeave={() => setHoveringWound(false)}
                    />
                    ...
                    <path
                        ...
                        style="fill: rgba(255, 103, 103, 0);"
                        onMouseEnter={() => handleWoundHover('rightArm')}
                        onMouseLeave={() => setHoveringWound(false)}
                    />
                    ...
                    <path
                        ...
                        style="fill: rgba(255, 103, 103, 0);"
                        onMouseEnter={() => handleWoundHover('leftArm')}
                        onMouseLeave={() => setHoveringWound(false)}
                    />
                    ...
                    <path
                        ...
                        style="fill: rgba(255, 103, 103, 0);"
                        onMouseEnter={() => handleWoundHover('rightLeg')}
                        onMouseLeave={() => setHoveringWound(false)}
                    />
                    ...
                    <path
                        ...
                        style="fill: rgba(255, 103, 103, 0);"
                        onMouseEnter={() => handleWoundHover('leftLeg')}
                        onMouseLeave={() => setHoveringWound(false)}
                    />
                </svg>
            </div>
            ...
            <div class="status head">
                ...
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.head.damage / woundState()?.wounds.head.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
                    ...
                    class="mt-[0.1vh]"
                >
                    <path d="M58 0V13.5H1.5" stroke="white" stroke-opacity="0.25" />
                    <rect x="57" width="2" height="2" fill="white" />
            ...
            <div class="status body">
                ...
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.body.damage / woundState()?.wounds.body.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
            ...
            <div class="status leftArm">
                Left Arm
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.leftArm.damage / woundState()?.wounds.leftArm.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
            ...
            <div class="status rightArm">
                Right Arm
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.rightArm.damage / woundState()?.wounds.rightArm.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
            ...
            <div class="status leftLeg">
                Left Leg
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.leftLeg.damage / woundState()?.wounds.leftLeg.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
            ...
            <div class="status rightLeg">
                Right Leg
                <div class="healthBar">
                    <div class="bar">
                        <div class="progress" style={{ width: `${woundState()?.wounds.rightLeg.damage / woundState()?.wounds.rightLeg.maxDamage * 100 || 0}%` }} />
                    </div>
                </div>
                <svg
                    width="5.46vh"
