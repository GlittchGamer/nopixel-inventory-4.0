...
export const CreateSlot = (props: any) => {
    ...
    const calculateBackgroundColor = () => {
        ...
        if (draggingItem.Item && draggingItem.isDragging) {
            for (const [invType, acceptedItems] of Object.entries(inventoryRestrictions)) {
                if (activeInventory().includes(invType)) {
                    const isAccepted = acceptedItems.some(item => 
                        draggingItem.Item.name.toLowerCase() === item.toLowerCase()
                    );
                    
                    setAcceptingItem(isAccepted);
                    return isAccepted ? 
                        'background:radial-gradient(83%_83%_at_50%_50%,_rgba(0,_248,_185,_0.15),_rgba(0,_102,_76,_0.25))' : 
                        'background:radial-gradient(83%_83%_at_50%_50%,_rgba(255,_0,_0,_0.15),_rgba(102,_0,_0,_0.25))';
                }
            }
        }
    ...
    const calculateBorderColor = () => {
        if (activeInventory().includes('wallet') && draggingItem.Item !== null && draggingItem.isDragging) {
            const walletAcceptedItems = ['cash', 'id_card'];
            
            const isAccepted = walletAcceptedItems.some(item => 
                draggingItem.Item.name.toLowerCase() === item.toLowerCase()
            );
            
            return isAccepted ? 'border:1px_solid_#00F8B9' : 'border:1px_solid_#FF0000';
        }
        
        if (activeInventory().includes('simcard-') && draggingItem.Item !== null && draggingItem.isDragging) {
            const simcardAcceptedItems = ['Sim Card'];
            
            const isAccepted = simcardAcceptedItems.some(item => 
                draggingItem.Item.name.toLowerCase() === item.toLowerCase()
            );
