...
export const Backpack = () => {
    ...
    const [hasBackpack, setHasBackpack] = createSignal(false);
    
    createEffect(() => {
        let backpackFound = false;
        
        if (!backpackFound && Inventory?.ClothingSlots) {
            const bagSlot = Inventory.ClothingSlots.find((slot: any) => {
                const exists = slot.item ? slot.item.itemId == 'bag' : false;
                return slot.id === 'bag' || exists
            });
            
            if (bagSlot) {
                backpackFound = bagSlot.item !== null;
            }
        } else if (!backpackFound) {
            console.log(`ClothingSlots is not available`);
        }
        
        setHasBackpack(backpackFound);
    });

    return (
        <Show when={hasBackpack()}>
            <div
