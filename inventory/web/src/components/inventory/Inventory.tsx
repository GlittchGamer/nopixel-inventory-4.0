...
import { Slot } from './Components/ActionBar/Components/Slot/Slot'
...
export const Inventory = () => {
    ...
    return (
        ...
        >
            ...
            <Show
                when={hoveringWound() && show() && activeWoundData()}
            >
                <div class="infoWounds" style={`left: calc(${position().x}px); top: calc(${position().y}px); margin-left: ${woundPosition() === 'left' ? `-24.33vh` : `1.6vh`}`}>
                    {activeWoundData()?.label || 'Information'}

                    <div class="infoItem"> Bullets
                        <div class="infoResult">{activeWoundData()?.bullets || 0}</div>
                    </div>
                    
                    <div class="infoItem"> Broken
                        <div class="infoResult">{activeWoundData()?.broken ? 'Yes' : 'No'}</div>
                    </div>
                    
                    <div class="infoItem">Severity 
                        <div class="infoResult">{activeWoundData()?.severity?.name || 'None'}</div>
                    </div>

                    <div class="infoItem"> Bleeding
                        <div class="infoResult">{activeWoundData()?.bleeding ? 'Yes' : 'No'}</div>
                    </div>
                </div>
            </Show>
                            ...
                            >
                                <div
                                    ...
                                    onClick={() => {
                                        nuiAction('useItem', {
                                            ItemId: hoveringItem.Information.itemId,
                                            ItemData: hoveringItem.Information,
                                            inventoryName: hoveringItem.fromInventory,
                                            isSideSlot: hoveringItem.isSideSlot,
                                            slot: hoveringItem.isSideSlot ? hoveringItem.slot : hoveringItem.fromSlot,
                                            fromInventory: hoveringItem.isSideSlot ? null : hoveringItem.fromInventory,
                                            itemAction: itemList[hoveringItem.Information.itemId].context.action
