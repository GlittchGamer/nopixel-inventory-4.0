...
onNet('RunUseItem', async function (itemid: any, slot: number, inventoryName: string, WeaponHash: any, passedItemInfo: any) {
    ...
    if (itemid === "pistolammoPD") {
        const finished = await exports["np-taskbar"].taskBar(2500, "Reloading", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === 'murdermeal') {
        const finished = exports['np-taskbar'].taskBar(1000, 'Unwrapping');
        if (finished === 100) {
    ...
    if (itemid == "murdermeal") {
        const finished = await exports['np-taskbar'].taskBar(1000, 'Unwrapping');
    ...
    if (itemid === "lsdtab" || itemid === "badlsdtab") {
        ...
        const finished = exports["np-taskbar"].taskBar(3000, "Placing LSD Strip on 👅", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "decryptersess" || itemid === "decrypterfv2" || itemid === "decrypterenzo") {
        if (GetDistanceBetweenCoords(GetEntityCoords(player)[0], GetEntityCoords(player)[1], GetEntityCoords(player)[2], 1275.49, -1710.39, 54.78, true) < 3.0) {
            const finished = exports["np-taskbar"].taskBar(25000, "Decrypting Data", false, false, playerVeh);
            if (finished === 100) {
    ...
    if (itemid === "smallbud" && hasEnoughOfItem("qualityscales", 1, false, false, false)) {
        const finished = exports["np-taskbar"].taskBar(1000, "Packing Joint", false, false, playerVeh);
        if (finished === 100) {
        }
    }

    if (itemid === "weedq") {
        const finished = exports["np-taskbar"].taskBar(1000, "Rolling Joints", false, false, playerVeh);
        if (finished === 100) {
        }
    }
    ...
    if (itemid === "lighter") {
        ...
        const finished = await exports["np-taskbar"].taskBar(2000, "Starting Fire", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "joint" || itemid === "joint2" || itemid === "customjointitem") {
        const finished = await exports["np-taskbar"].taskBar(2000, "Smoking Joint", false, false, playerVeh);
        let smoking = false;
        if (finished === 100) {
            remove = true;
            smoking = true;
    ...
    if (itemid === "clotion" || itemid === "cabsinthe") {
        remove = true;
        const finished = await exports["np-taskbar"].taskBar(2500, "Consuming Lotion");
        if (finished === 100) {
            SetPlayerMaxArmour(PlayerPedId(), 100);
        ...
        if (Police >= 4) {
            if (targetVehicle !== 0 && GetHashKey("stockade") === GetEntityModel(targetVehicle)) {
                ...
                if (aDist < 2.0) {
                    ...
                    const finished = exports["np-taskbar"].taskBar(timeToOpen, "Unlocking Vehicle", false, false, playerVeh);
                    if (finished === 100) {
    ...
    if (itemid === "heavyammo") {
        const finished = await exports["np-taskbar"].taskBar(5000, "Reloading", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "flamethrowerammo") {
        const finished = await exports["np-taskbar"].taskBar(20000, "Reloading flamethrower", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "huntingammo") {
        const finished = await exports["np-taskbar"].taskBar(10000, "Reloading", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "rpgammo") {
        const finished = await exports["np-taskbar"].taskBar(25000, "Reloading", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "taserammo") {
        const finished = await exports["np-taskbar"].taskBar(2000, "Reloading", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "empammo") {
        const finished = await exports["np-taskbar"].taskBar(30000, "Recharging EMP", false, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "armor" || itemid === "pdarmor") {
        const finished = await exports["np-taskbar"].taskBar(4500, "Putting on Armor", true, false, playerVeh);
        if (finished === 100) {
    ...
    if (itemid === "nitrous") {
        ...
        (async () => {
            finished = await exports["np-taskbar"].taskBar(20000, "Nitrous");
            if (finished === 100 && !cancelNos) {
...
async function TaskItem(
    ...
    if (timerNum > 0) {
        const finished = await exports["np-taskbar"].taskBar(timerNum, message, true, false, playerVeh)
