...
class Controller {
  ...
  constructor() {
    ...
    this._hashing = new Hashing(token, "0xB21A8464"); // Replace with actual token value ? nahh
  }
  ...
  onNet(event: string, callback: (...args: any[]) => void): void {
    ...
    onNet(hashCompressed, (payload: Uint8Array) => {
      const decompressed: Uint8Array = Utils.inflate(payload);
