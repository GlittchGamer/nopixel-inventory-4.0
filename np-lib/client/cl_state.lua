...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports["qb-flags"]:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.flags = exports["qb-flags"]:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports["qb-flags"]:GetObjectFlags(pEntity)
    end
...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags["isJobEmployer"] then
        pContext.job = exports["qb-jobs"]:GetNPCJobData(npcId)
    end
end

--[[function isDisabled()
    return exports["qb-base"]:getVar("dead") or
        exports["qb-base"]:getVar("handcuffed")
end]]
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports["qb-vehicles"]:HasVehicleKey(pEntity)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    if zone == "qb-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
RegisterNetEvent("qb-jobs:jobChanged")
AddEventHandler("qb-jobs:jobChanged", function(pJobId)
    CurrentJob = pJobId
