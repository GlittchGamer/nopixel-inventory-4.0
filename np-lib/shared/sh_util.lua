if not IsDuplicityVersion() then
    ...
    function SetCustomNuiFocus(hasKeyboard, hasMouse)
        ...
        TriggerEvent("qb-base:focus", HasNuiFocus, hasKeyboard, hasMouse)
    end
    ...
    function GetNearbyPeds(X, Y, Z, Radius)
        ...
        if tonumber(X) and tonumber(Y) and tonumber(Z) then
            if tonumber(Radius) then
                for Ped in exports["qb-admin"]:EnumeratePeds() do
    ...
    function SkillCircleLooped(count)
        ...
        while loop < count do
            loop = loop + 1
            local finished = exports["qb-skillcircle"]:SkillCircle(math.random(13, 60))
    ...
    function SpeedSkillCircle()
        local finished = exports["qb-skillcircle"]:SkillCircle(math.random(4, 7))
        if not finished then
            return false
        end
        return true
    ...
    function SkillCircleLooped2(sent)
        ...
        for i = 1, #speeds do
            local finished = exports["qb-skillcircle"]:SkillCircle(speeds[i])
            if not finished then
                return false
            end
            Wait(200)
    ...
    function SpawnVehicle(model, spawn, heading)
        ...
        TriggerEvent('qb-vmod:addKeys', _vehicle)
...
function encodeAccountId(plain)
    if type(plain) == 'number' then
        return math.floor(exports["qb-financials"]:EncodeId(plain))
    end
end

function decodeAccountId(encoded)
    if type(encoded) == 'number' then
        return exports["qb-financials"]:DecodeId(encoded)
    end
end
